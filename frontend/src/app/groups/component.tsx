...
export default function StudentGroupsPage({ orgs }: StudentGroupsPageProps) {
  ...
  // Modal state
  const [selectedOrg, setSelectedOrg] = useState<OrganisationView | null>(null);
  ...
  useEffect(() => {
    const inactiveFilteredCards = orgs.filter(
    ...
    setPage(1);
  }, [interestGroupFilters, interestGroupSearchString, orgs, isInactive]);
  ...
  const handleShowMore = (org: OrganisationView) => {
    setSelectedOrg(org);
    setIsModalOpen(true);
  };
