...
export default function UsersPage({ users, organisations }: UsersPageProps) {
  ...
  const [editUserState, editUserAction, editUserPending] = useActionState(editUser, null)
  const [deleteUserState, deleteUserAction, deleteUserPending] = useActionState(deleteUser, null)
  ...
  const handleEditSubmit = (formData: z.input<typeof UpdateUserSchema>) => {
    ...
    formData.organisationIds.forEach((orgId) => editOrganisation.append("organisationIds", orgId.toString()))
  ...
  const paginateArray = (pageNumber: number) => {
    return filtedUsers.slice((pageNumber - 1) * PAGE_SIZE, pageNumber * PAGE_SIZE)
  }
            ...
            <div key={user.id} className="px-6 py-4 transition-colors hover:bg-gray-50">
              <div className="grid grid-cols-1 items-center gap-4 md:grid-cols-4">
                <div className="font-medium text-gray-900">
                  {user.name}
                  <div className="mt-1 text-sm text-gray-500 md:hidden">{user.telegramUserName}</div>
                  <div className="mt-1 text-sm text-gray-500 md:hidden">
                    {user.userOrgs.map((userOrg) => userOrg.org.name).join(", ")}
                  </div>
                </div>
                <div className="hidden text-gray-600 md:block">{user.telegramUserName}</div>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={() => handleDeleteSubmit(user.id)}>Continue</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                </div>
